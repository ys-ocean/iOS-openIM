...
//#warning TODO: CHANGE TO YOUR AppKey
        /// 线上环境，更换成你自己的AppKey
        [[YWAPI sharedInstance] syncInitWithOwnAppKey:CMAPPKEY getError:&error];
    } else {
 ...
 *  预登陆
 */
- (void)examplePreLoginWithLoginId:(NSString *)loginId successBlock:(void(^)(void))aPreloginedBlock
{
    /// 预登录
...
- (void)exampleListenNewMessage
{
    [[self.ywIMKit.IMCore getConversationService] addOnNewMessageBlockV2:^(NSArray *aMessages, BOOL aIsOffline) {
        ...
        /// 发送通知更新会话列表界面 如果在会话列表界面才需要(展示列表界面的时候 会默认更新)
        UIViewController * vc = self.rootWindow.rootViewController;
        if ([vc isKindOfClass:[UITabBarController class]]) {
            if (((UITabBarController *)vc).viewControllers.count > CMMsgVCIndex) {
                UINavigationController * nav = ((UITabBarController *)vc).viewControllers[CMMsgVCIndex];
                if ([nav.topViewController isKindOfClass:[NSClassFromString(@"CarMsgListViewController") class]]) {
                    [[NSNotificationCenter defaultCenter]postNotificationName:CMFetchNewMessageKey object:nil userInfo:@{CMFetchNewMessageValueKey:aMessages}];
                }
            }
        }
        else if ([vc isKindOfClass:[UINavigationController class]]) {
            if ([((UINavigationController *)vc).topViewController isKindOfClass:[NSClassFromString(@"CarMsgListViewController") class]]) {
                [[NSNotificationCenter defaultCenter]postNotificationName:CMFetchNewMessageKey object:nil userInfo:@{CMFetchNewMessageValueKey:aMessages}];
            }
        }
